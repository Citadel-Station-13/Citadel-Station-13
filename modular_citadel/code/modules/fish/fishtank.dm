...
/obj/machinery/fishtank/wall
	...
	density = 1
	anchored = 1
	pass_flags = 0				// This thing literally a fragile fish-wall, you can't throw past it.
			...
			//Empty containers will scoop out water, filling the container as much as possible from the water_level
				...
				if(water_level == 0)
					...
					if(water_level >= C.volume)										//Enough to fill the container completely
						...
						user.visible_message("[user.name] scoops out some water from \the [src].", "You completely fill \the [C.name] from \the [src].")
						...
						user.visible_message("[user.name] scoops out some water from \the [src].", "You fill \the [C.name] with the last of the water in \the [src].")
			return
