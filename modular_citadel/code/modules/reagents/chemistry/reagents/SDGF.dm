	...
	switch(current_cycle)
		if(1)
			...
			if(pollStarted == FALSE)
				pollStarted = TRUE
				candies = pollGhostCandidates("Do you want and agree to play as a clone of [M], respect their character and not engage in ERP as them without permission from the original?", ignore_category = POLL_IGNORE_CLONE)
