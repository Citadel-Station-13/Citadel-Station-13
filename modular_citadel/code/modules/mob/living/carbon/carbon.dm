...
/mob/living/carbon/proc/toggle_combat_mode()
	...
	client?.toggle_popup_menus(!combatmode) // So we can right-click for alternate actions and all that other good shit. Also moves examine to shift+rightclick to make it possible to attack while sprinting
