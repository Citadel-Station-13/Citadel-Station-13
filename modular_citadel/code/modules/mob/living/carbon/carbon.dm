...
/mob/living/carbon/proc/toggle_combat_mode()
	...
	if(world.time >= combatmessagecooldown && combatmode)
		visible_message("<span class='warning'>[src] [resting ? "tenses up" : (prob(50)? "drops into a combative stance" : (prob(99)? "poses aggressively" : "asserts dominance with their pose"))].</span>")
