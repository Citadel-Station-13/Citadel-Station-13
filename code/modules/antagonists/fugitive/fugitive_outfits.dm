...
/datum/outfit/waldo/post_equip(mob/living/carbon/human/H, visualsOnly=FALSE)
	...
	no_drops += H.get_item_in_slot(ITEM_SLOT_FEET)
	no_drops += H.get_item_in_slot(ITEM_SLOT_ICLOTHING)
	no_drops += H.get_item_in_slot(ITEM_SLOT_OCLOTHING)
	no_drops += H.get_item_in_slot(ITEM_SLOT_HEAD)
	no_drops += H.get_item_in_slot(ITEM_SLOT_EYES)
