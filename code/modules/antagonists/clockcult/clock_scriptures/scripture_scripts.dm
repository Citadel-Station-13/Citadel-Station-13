...
/datum/action/innate/clockwork_armaments/Activate()
	var/do_message = 0
	var/obj/item/I = owner.get_item_in_slot(INVENTORY_SLOT_OUTERWEAR)
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/suit/armor/clockwork(null), INVENTORY_SLOT_OUTERWEAR)
	I = owner.get_item_in_slot(INVENTORY_SLOT_HEAD)
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/head/helmet/clockwork(null), INVENTORY_SLOT_HEAD)
	I = owner.get_item_in_slot(INVENTORY_SLOT_GLOVES)
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/gloves/clockwork(null), INVENTORY_SLOT_GLOVES)
	I = owner.get_item_in_slot(INVENTORY_SLOT_SHOES)
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/shoes/clockwork(null), INVENTORY_SLOT_SHOES)
	if(do_message)
	...
	for(var/atom/movable/M in range(7, invoker))
		if(isliving(M))
			if(isclockmob(M) || istype(M, /mob/living/simple_animal/drone/cogscarab))
				...
				var/obj/item/I = H.get_item_in_slot(INVENTORY_SLOT_OUTERWEAR)
				if(is_type_in_typecache(I, ratvarian_armor_typecache))
					heal_ticks++
				I = H.get_item_in_slot(INVENTORY_SLOT_HEAD)
				if(is_type_in_typecache(I, ratvarian_armor_typecache))
					heal_ticks++
				I = H.get_item_in_slot(INVENTORY_SLOT_GLOVES)
				if(is_type_in_typecache(I, ratvarian_armor_typecache))
					heal_ticks++
				I = H.get_item_in_slot(INVENTORY_SLOT_SHOES)
				if(is_type_in_typecache(I, ratvarian_armor_typecache))
					heal_ticks++
				if(heal_ticks)
