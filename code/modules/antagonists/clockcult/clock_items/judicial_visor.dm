...
/obj/item/clothing/glasses/judicial_visor/item_action_slot_check(slot, mob/user, datum/action/A)
	if(slot != INVENTORY_SLOT_GLASSES)
		return 0
	return ..()
...
/obj/item/clothing/glasses/judicial_visor/proc/check_on_mob(mob/user)
	if(user && src != user.get_item_in_slot(INVENTORY_SLOT_GLASSES)) //if we happen to check and we AREN'T in the slot, we need to remove our shit from whoever we got dropped from
		update_status(FALSE)
		if(blaster.ranged_ability_user)
			blaster.remove_ranged_ability()

/obj/item/clothing/glasses/judicial_visor/attack_self(mob/user)
	if(is_servant_of_ratvar(user) && src == user.get_item_in_slot(INVENTORY_SLOT_GLASSES))
		blaster.toggle(user)
...
/obj/item/clothing/glasses/judicial_visor/proc/recharge_visor(mob/living/user)
	if(!src)
		return 0
	recharging = FALSE
	if(user && src == user.get_item_in_slot(INVENTORY_SLOT_GLASSES))
...
/obj/effect/proc_holder/judicial_visor/InterceptClickOn(mob/living/caller, params, atom/target)
	if(..())
		return
	if(ranged_ability_user.incapacitated() || !visor || visor != ranged_ability_user.get_item_in_slot(INVENTORY_SLOT_GLASSES))
