...
// Creates the gang team, or sets it. The objectives are static for all the team members.
/datum/antagonist/gang/create_team(datum/team/overthrowers)
	if(!overthrowers)
		team = new()
...
/datum/antagonist/gang/apply_innate_effects()
	..()
...
// The opposite
/datum/antagonist/gang/remove_innate_effects()
...
/datum/antagonist/gang/get_admin_commands()
	...
	.["Give gang boss equip"] = CALLBACK(src,.proc/equip_initial_overthrow_agent)
...
/datum/antagonist/gang/proc/update_overthrow_icons_added(datum/mind/traitor_mind)
...
/datum/antagonist/gang/proc/update_overthrow_icons_removed(datum/mind/traitor_mind)
	var/datum/atom_hud/antag/overthrowhud = GLOB.huds[team.hud_entry_num]
	if(overthrowhud)
...
/datum/antagonist/gang/proc/equip_overthrow()
	if(!owner || !owner.current || !ishuman(owner.current)) // only equip existing human gang members. This excludes the AI, in particular.
		return
...
// Equip the initial gang agent. Manually called in gang gamemode, when the initial agents are chosen. Gives uplink, AI module board and the converter.
/datum/antagonist/gang/proc/equip_initial_overthrow_agent()
	...
	var/obj/item/aiModule/core/full/gang/O = new(H)
	var/list/slots = list (
