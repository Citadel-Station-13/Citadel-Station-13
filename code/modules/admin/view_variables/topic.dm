...
//Stuff below aren't in dropdowns/etc.
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		if(href_list["rename"])
			...
			switch(Text)
				if("brute")
					L.adjustBruteLoss(amount, cause = "killed by badminnery")
					newamt = L.getBruteLoss()
				if("fire")
					L.adjustFireLoss(amount, cause = "killed by badminnery")
					newamt = L.getFireLoss()
				if("toxin")
					L.adjustToxLoss(amount, toxins_type = TOX_OMNI, forced = TRUE, cause = "killed by badminnery")
					newamt = L.getToxLoss()
				if("oxygen")
					L.adjustOxyLoss(amount, cause = "killed by badminnery")
					newamt = L.getOxyLoss()
				if("brain")
					L.adjustOrganLoss(ORGAN_SLOT_BRAIN, amount, cause = "killed by badminnery")
					newamt = L.getOrganLoss(ORGAN_SLOT_BRAIN)
				if("clone")
					L.adjustCloneLoss(amount, cause = "killed by badminnery")
