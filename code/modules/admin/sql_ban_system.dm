...
/datum/admins/proc/ban_panel(player_key, player_ip, player_cid, role, duration = 1440, applies_to_admins, reason, edit_id, page, admin_key)
	...
	if(edit_id)
		...
		var/list/long_job_lists = list("Civilian" = GLOB.civilian_positions,
									"Ghost and Other Roles" = list(ROLE_BRAINWASHED, ROLE_DEATHSQUAD, ROLE_DRONE, ROLE_GHOSTCAFE, ROLE_LAVALAND, ROLE_MIND_TRANSFER, ROLE_POSIBRAIN, ROLE_SENTIENCE),
									"Antagonist Positions" = list(ROLE_ABDUCTOR, ROLE_ALIEN, ROLE_BLOB,
									ROLE_BLOODSUCKER, ROLE_BROTHER, ROLE_CHANGELING, ROLE_CULTIST,
									ROLE_DEVIL, ROLE_GANG, ROLE_INTERNAL_AFFAIRS, ROLE_MALF,
									ROLE_MINOR_ANTAG, ROLE_MONKEY, ROLE_NINJA, ROLE_OPERATIVE,
