...
/client/proc/place_ruin()
	...
	seedRuins(SSmapping.LevelsByTrait(data[2]), max(1, template.cost), data[3], list(ruinname = template))
...
/client/proc/clear_dynamic_transit()
	...
	if(!holder)
		return
	var/answer = alert("WARNING: THIS WILL WIPE ALL RESERVED SPACE TO A CLEAN SLATE! ANY MOVING SHUTTLES, ELEVATORS, HOTEL ROOMS, OR IN-PROGRESS PHOTOGRAPHY WILL BE DELETED!", "Really wipe dynamic turfs?", "YES", "NO")
	if(answer != "YES")
		return
	...
	SSmapping.WipeSpaceReservations()				//this goes after it's logged, incase something horrible happens.
