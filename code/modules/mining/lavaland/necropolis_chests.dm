...
/obj/item/melee/ghost_sword/upgraded/attack(mob/living/target, mob/living/carbon/human/user)
	...
	force = clamp((ghost_counter * 8), 0, 75)
	user.visible_message("<span class='danger'>[user] strikes with the force of [ghost_counter] vengeful spirits!</span>")
	return ..()

/obj/item/melee/ghost_sword/upgraded/run_block(mob/living/owner, atom/object, damage, attack_text, attack_type, armour_penetration, mob/attacker, def_zone, final_block_chance, list/block_return)
	var/ghost_counter = ghost_check()
	final_block_chance += clamp((ghost_counter * 10), 0, 75)
	owner.visible_message("<span class='danger'>[owner] is protected by a ring of [ghost_counter] ghosts!</span>")
	return ..()
