...
GLOBAL_LIST_INIT(ballmer_good_msg, list("Hey guys, what if we rolled out a bluespace wiring system so mice can't destroy the powergrid anymore?",
										...
										"Why the fuck aren't we just making every pair of shoes into galoshes? We have the technology.",
										"We can link the Ore Silo to are prolathens, why not also link it to autolathens or Exo Fabs.",
										"If we can make better bombs with just some plasma and tritium then why still have station nukes use plutonium.",
										"The modular console can do some functions like monitor the Super Matter or fix an AI then why not make more programs for crew monitor or sec records.",
										"So I was thinking, if we more manipulators in something it prints for cheaper right? So what if we just made new printers that have like 12 manipulators  inside them to print stuff really cheap."
										))
GLOBAL_LIST_INIT(ballmer_windows_me_msg, list("Yo man, what if, we like, uh, put a webserver that's automatically turned on with default admin passwords into every PDA?",
											...
											"Bruh what if we took a locker, some random trash and made a mech out of it?",
											"Ok here me out, what if we make illegal, not illegal so sec stops arresting us for having it.",
											"Crazy idea, ok ok ok, rather then having monkeys to test on, what if we only used apes.",
											"Woh man ok, what if we took slime cores and smashed them into other slimes, be kinda cool to see what happens.",
											"Were Nano-trasen but we need to unlock nano-parts whats the deal with that?"
											))
