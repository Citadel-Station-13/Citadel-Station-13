...
/mob/living/carbon/human/PhysicalLife(seconds, times_fired)
	...
	if(ckey == "Silicons" || "CygnusB" || "Putnam" || "R3dtaile" || "DeltaFire" || "R0-adic" || "Xantholne" || "Trojancoyote" || "Dzahlus" || "NicoDeimos" || "Bhijn" ) // Don't know if nico is still an admin, don't have an admin list anywhere so feel free to add if there's people missing
