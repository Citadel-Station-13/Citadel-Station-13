	...
	for(var/layer in relevant_layers)
		...
		for(var/bodypart in relevant_layers[layer])
			...
			if(S.marking_state)
				...
				marking_overlay.icon_state = S.marking_state
				var/marking_feature = "[S.mutant_part_string]_marking_color"
			...
			if(S.center)
				...
				if(marking_overlay)
					marking_overlay = center_image(marking_overlay, S.dimension_x, S.dimension_y)
				...
				if(!forced_colour)
					switch(S.color_src)
						...
						if(DUAL_COLOR)
							...
							//the main part is done here, the marking is already handled
							var/accessory_feature = "[S.mutant_part_string]_color"
							accessory_overlay.color = "#[H.dna.features[accessory_feature]]"
				else
					accessory_overlay.color = forced_colour
					if(marking_overlay)
						marking_overlay.color = forced_colour
			else
				if(bodypart == "ears")
