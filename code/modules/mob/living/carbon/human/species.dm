...
//END EDIT
	...
	for(var/layer in relevent_layers)
		...
		for(var/bodypart in bodyparts_to_add)
			...
			var/color_r = GetHexColors("#[H.dna.features["mcolor"]]")
			var/color_g = GetHexColors("#[H.dna.features["mcolor2"]]")
			var/color_b = GetHexColors("#[H.dna.features["mcolor3"]]")
			...
			if(!(H.has_trait(TRAIT_HUSK)))
				if(!forced_colour)
					switch(S.color_src)
						if(MUTCOLORS)
							if(fixed_mut_color)
								accessory_overlay.color = list(fcolor_r,fcolor_g,fcolor_b,list(0, 0, 0))
							else
								accessory_overlay.color = list(color_r,color_g,color_b,list(0, 0, 0))
/*						if(MUTCOLORS2)
							if(fixed_mut_color2)
