...
/mob/living/carbon/show_inv(mob/user)
	...
	<HR>
	<BR><B>Head:</B> <A href='?src=[REF(src)];item=[INVENTORY_SLOT_HEAD]'>				[(head && !(head.item_flags & ABSTRACT)) 			? head 		: "Nothing"]</A>
	<BR><B>Mask:</B> <A href='?src=[REF(src)];item=[INVENTORY_SLOT_MASK]'>		[(wear_mask && !(wear_mask.item_flags & ABSTRACT))	? wear_mask	: "Nothing"]</A>
	<BR><B>Neck:</B> <A href='?src=[REF(src)];item=[INVENTORY_SLOT_NECK]'>		[(wear_neck && !(wear_neck.item_flags & ABSTRACT))	? wear_neck	: "Nothing"]</A>"}

	for(var/i in 1 to held_items.len)
		var/obj/item/I = get_item_for_held_index(i)
		dat += "<BR><B>[get_held_index_name(i)]:</B></td><td><A href='?src=[REF(src)];item=[INVENTORY_SLOT_PUT_IN_HANDS];hand_index=[i]'>[(I && !(I.item_flags & ABSTRACT)) ? I : "Nothing"]</a>"

	dat += "<BR><B>Back:</B> <A href='?src=[REF(src)];item=[INVENTORY_SLOT_BACK]'>[back ? back : "Nothing"]</A>"

	if(!HAS_TRAIT(src, TRAIT_NO_INTERNALS) && istype(wear_mask, /obj/item/clothing/mask) && istype(back, /obj/item/tank))
		dat += "<BR><A href='?src=[REF(src)];internal=1'>[internal ? "Disable Internals" : "Set Internals"]</A>"

	if(handcuffed)
		dat += "<BR><A href='?src=[REF(src)];item=[INVENTORY_SLOT_HANDCUFFED]'>Handcuffed</A>"
	if(legcuffed)
		dat += "<BR><A href='?src=[REF(src)];item=[INVENTORY_SLOT_LEGCUFFED]'>Legcuffed</A>"
...
/mob/living/carbon/Topic(href, href_list)
	...
	if(usr.canUseTopic(src, BE_CLOSE))
		if(href_list["internal"] && !HAS_TRAIT(src, TRAIT_NO_INTERNALS))
			...
			var/obj/item/ITEM = get_item_in_slot(slot)
...
/mob/living/carbon/resist_buckle()
	...
	if(restrained())
		...
		var/buckle_cd = 600
		if(handcuffed)
			var/obj/item/restraints/O = src.get_item_in_slot(INVENTORY_SLOT_HANDCUFFED)
...
/mob/living/carbon/check_obscured_slots()
	if(head)
		if(head.flags_inv & HIDEMASK)
			LAZYOR(., INVENTORY_SLOT_MASK)
		if(head.flags_inv & HIDEEYES)
			LAZYOR(., INVENTORY_SLOT_GLASSES)
		if(head.flags_inv & HIDEEARS)
			LAZYOR(., INVENTORY_SLOT_EARS)
