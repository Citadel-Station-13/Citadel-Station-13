	...
	if(!user.is_muzzled() && !miming)
		...
		if(issilicon(user))
			...
			if(iscyborg(user))
				...
				S.cell.use(200)
		if(ismonkey(user))
			sound = 'sound/voice/scream/scream_monkey.ogg'
		if(istype(user, /mob/living/simple_animal/hostile/gorilla))
			sound = 'sound/creatures/gorilla.ogg'
		if(ishuman(user))
			user.adjustOxyLoss(5)
			sound = pick('sound/voice/scream/scream_m1.ogg', 'sound/voice/scream/scream_m2.ogg')
			if(user.gender == FEMALE)
			...
			if(is_species(user, /datum/species/skeleton))
				sound = 'sound/voice/scream/scream_skeleton.ogg'
			if (is_species(user, /datum/species/fly) || is_species(user, /datum/species/insect))
				sound = 'sound/voice/scream/scream_moth.ogg'
		if(isalien(user))
