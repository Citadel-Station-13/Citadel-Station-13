...
/mob/living/simple_animal/parrot
	...
	turns_per_move = 5
	butcher_results = list(/obj/item/reagent_containers/food/cracker/ = 1)
...
/mob/living/simple_animal/parrot/attackby(obj/item/O, mob/living/user, params)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O, /obj/item/reagent_containers/food/cracker))
		if(O.force)
			if(parrot_state == PARROT_PERCH)
				parrot_sleep_dur = parrot_sleep_max //Reset it's sleep timer if it was perched
			...
			icon_state = icon_living
			drop_held_item(0)
	else if(istype(O, /obj/item/reagent_containers/food/cracker)) //Poly wants a cracker.
		qdel(O)
		if(health < maxHealth)
			adjustBruteLoss(-10)
...
//parrots will eat crackers instead of dropping them
	if(istype(held_item, /obj/item/reagent_containers/food/cracker) && (drop_gently))
		qdel(held_item)
		held_item = null
		if(health < maxHealth)
			adjustBruteLoss(-10)
