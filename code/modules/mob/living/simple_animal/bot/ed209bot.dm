...
/mob/living/simple_animal/bot/ed209/handle_automated_action()
	...
	switch(mode)
		...
		if(BOT_PREP_ARREST)		// preparing to arrest target
			...
			if(!Adjacent(target) || !isturf(target.loc) || !(target.combat_flags & COMBAT_FLAG_HARD_STAMCRIT) || target.getStaminaLosstotal() <= 120) // CIT CHANGE - replaces amountknockdown with recoveringstam and staminaloss checks
				back_to_hunt()
				return
		...
		if(BOT_ARREST)
			...
			if(!Adjacent(target) || !isturf(target.loc) || (target.loc != target_lastloc && !(target.combat_flags & COMBAT_FLAG_HARD_STAMCRIT) && target.getStaminaLosstotal() <= 120)) //if he's changed loc and about to get up or not adjacent or got into a closet, we prep arrest again. CIT CHANGE - replaces amountknockdown with recoveringstam and staminaloss checks
				back_to_hunt()
				return
			else
				mode = BOT_PREP_ARREST
