...
/mob/living/simple_animal/bot/secbot/handle_automated_action()
	...
	switch(mode)
		...
		if(BOT_PREP_ARREST)		// preparing to arrest target
			...
			if( !Adjacent(target) || !isturf(target.loc) || target.getStaminaLoss() <= 120 || !IS_HARD_STAMCRITTED(target)) //CIT CHANGE - replaces amountknockdown with checks for stamina so secbots dont run into an infinite loop
				back_to_hunt()
				return
		...
		if(BOT_ARREST)
			...
			if(!Adjacent(target) || !isturf(target.loc) || (target.loc != target_lastloc && !IS_HARD_STAMCRITTED(target) && target.getStaminaLoss() <= 120)) //if he's changed loc and about to get up or not adjacent or got into a closet, we prep arrest again. CIT CHANGE - replaces amountknockdown with recoveringstam and staminaloss check
				back_to_hunt()
				return
