...
/client/proc/admin_cmd_respawn_return_to_lobby()
	...
	for(var/mob/dead/new_player/I in GLOB.dead_mob_list)
		if(!I.client || !I.client.persistent_variables.respawn_restrictions_active)
			continue
	...
	if(istype(M, /mob/dead/observer))
		...
		O.do_respawn(FALSE)
		O.client.persistent_variables.dnr_triggered = FALSE
		...
		NP.client.persistent_variables.respawn_restrictions_active = FALSE
		NP.client.persistent_variables.dnr_triggered = FALSE
...
/client/proc/admin_cmd_remove_ghost_respawn_timer()
	...
	O.client.persistent_variables.respawn_time_of_death = -INFINITY
...
/mob/dead/new_player/proc/respawn_latejoin_check(notify = FALSE)
	if(!client.persistent_variables.respawn_restrictions_active)
		return TRUE
	...
	if(client.persistent_variables.slots_joined_as && (client.persistent_variables.default_slot in client.persistent_variables.slots_joined_as))
		if(notify)
			to_chat(src, "<span class='userdanger'>You cannot respawn on the same slot. Joined slots: [english_list(client.persistent_variables.slots_joined_as)].")
		return FALSE
	if((!nameless && !randomname) && (client.persistent_variables.characters_joined_as && (client.persistent_variables.real_name in client.persistent_variables.characters_joined_as)))
		if(notify)
			to_chat(src, "<span class='userdanger'>You cannot respawn on the same character. Joined slots: [english_list(client.persistent_variables.characters_joined_as)].")
		return FALSE
	return TRUE
...
/mob/dead/observer/verb/respawn()
	...
	if(client.persistent_variables.dnr_triggered)
...
/mob/dead/observer/verb/time_left_to_respawn()
	ASSERT(client)
	return max(0, ((client.persistent_variables.respawn_did_cryo? CONFIG_GET(number/respawn_delay_cryo) : CONFIG_GET(number/respawn_delay)) MINUTES + client.persistent_variables.respawn_time_of_death) - world.time)
...
/mob/dead/observer/proc/do_respawn(penalize)
	if(!client)
		return
	if(isnull(penalize))
		penalize = client.persistent_variables.respawn_restrictions_active
	client.persistent_variables.respawn_restrictions_active = penalize
