...
/mob/proc/doUnEquip(obj/item/I, force, newloc, no_move, invdrop = TRUE, new_sx = 0, new_sy = 0) //Force overrides TRAIT_NODROP for things like wizarditis and admin undress.
	...
	if(I)
		...
		if(!no_move && !(I.item_flags & DROPDEL))	//item may be moved/qdel'd immedietely, don't bother moving it
			if (isnull(newloc))
				...
				I.forceMove(newloc, new_sx, new_sy)
		I.dropped(src)
	return TRUE
