...
/datum/mafia_controller/proc/prepare_game(setup_list,ready_players)
	...
	var/turf/spawn_area = get_turf(locate(/atom/movable/landmark/mafia_game_area) in GLOB.landmarks_list)
	...
	if(!landmarks.len)//we grab town center when we grab landmarks, if there is none (the first game signed up for let's grab them post load)
		for(var/atom/movable/landmark/mafia/possible_spawn in GLOB.landmarks_list)
			if(istype(possible_spawn, /atom/movable/landmark/mafia/town_center))
