...
/obj/item/bodypart/r_arm/drop_limb(special)
	...
	if(C && !special)
		if(C.handcuffed)
			...
			C.update_handcuffed()
		if(C.hud_used)
			var/obj/screen/hand/R = C.hud_used.hand_slots["[held_index]"]
			if(R)
				R.update_icon()
		if(C.gloves)
...
/obj/item/bodypart/l_arm/drop_limb(special)
	...
	if(C && !special)
		if(C.handcuffed)
			...
			C.update_handcuffed()
		if(C.hud_used)
			var/obj/screen/hand/L = C.hud_used.hand_slots["[held_index]"]
			if(L)
				L.update_icon()
		if(C.gloves)
			C.dropItemToGround(C.gloves, TRUE)
...
/obj/item/bodypart/proc/attach_limb(mob/living/carbon/C, special)
	...
	if(held_index)
		...
		if(C.hud_used)
			var/obj/screen/hand/hand = C.hud_used.hand_slots["[held_index]"]
			if(hand)
				hand.update_icon()
		C.update_inv_gloves()
