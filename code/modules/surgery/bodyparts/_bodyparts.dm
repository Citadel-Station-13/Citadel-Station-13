...
/obj/item/bodypart/proc/receive_damage(brute = 0, burn = 0, stamina = 0, blocked = 0, updating_health = TRUE, required_status = null, wound_bonus = 0, bare_wound_bonus = 0, sharpness = SHARP_NONE) // maybe separate BRUTE_SHARP and BRUTE_OTHER eventually somehow hmm
	...
	*/
	...
	brute_dam += brute
	if(brute)
		INVOKE_ASYNC(owner, TYPE_PROC_REF(/atom, throw_damage_indicator), brute, BRUTE)
	burn_dam += burn
	if(burn)
		INVOKE_ASYNC(owner, TYPE_PROC_REF(/atom, throw_damage_indicator), burn, BURN)
	...
	if(stamina)
		INVOKE_ASYNC(owner, TYPE_PROC_REF(/atom, throw_damage_indicator), stamina, STAMINA)
