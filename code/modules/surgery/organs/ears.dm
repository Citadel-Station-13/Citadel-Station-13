...
/obj/item/organ/ears/on_life()
	...
	if((damage < maxHealth) && (isFailing()))	//ear damage can be repaired from the failing condition
		organ_flags &= ~ORGAN_FAILING
	// genetic deafness prevents the body from using the ears, even if healthy
	if(HAS_TRAIT(C, TRAIT_DEAF))
		deaf = max(deaf, 1)
	else if(!(isFailing())) // if this organ is failing, do not clear deaf stacks.
		deaf = max(deaf - 1, 0)
