...
/obj/structure/ladder/Destroy(force)
	...
	disconnect()
	if(listening)
		UnregisterSignal(listening, COMSIG_TURF_UPDATE_MULTIZ)
		listening = null
	return ..()
...
/obj/structure/ladder/LateInitialize()
	...
	if(isturf(loc))
		listening = loc
		RegisterSignal(loc, COMSIG_TURF_UPDATE_MULTIZ, .proc/Sync)

/obj/structure/ladder/Moved(atom/OldLoc, Dir)
	if(listening)
		UnregisterSignal(listening, COMSIG_TURF_UPDATE_MULTIZ)
	. = ..()
	listening = isturf(loc) && loc
	if(listening)
		RegisterSignal(listening, COMSIG_TURF_UPDATE_MULTIZ)
...
/obj/structure/ladder/proc/Sync()
	disconnect()
...
/obj/structure/ladder/PreventZFall(atom/movable/victim, levels, fall_flags)
	if(levels == 0)
		return TRUE
	return ..()
