/datum/preferences/proc/load_preferences(client/C)
	...
	while(query.NextRow())
		...
		tgui_fancy			= text2num(query.item[4])
		be_special			= params2list(query.item[5])
		chat_toggles		= params2list(query.item[6])
		toggles				= text2num(query.item[7])
		ghost_orbit			= query.item[8]
		ghost_form			= query.item[9]
		ghost_accs			= query.item[10]
		ghost_others		= query.item[11]
		preferred_map		= query.item[12]
		ignoring			= query.item[13]
		ghost_hud			= text2num(query.item[14])
		inquisitive_ghost	= text2num(query.item[15])
		uses_glasses_colour = text2num(query.item[16])
		clientfps			= text2num(query.item[17])
		parallax			= text2num(query.item[18])
		uplink_spawn_loc	= query.item[19]
		arousable			= query.item[20]
...
/datum/preferences/proc/load_character(client/C,slot)
	...
	while(query.NextRow())
		...
		default_slot		= text2num(query.item[35])
	...
	features["frills"]				= sanitize_inlist(features["frills"], frills_list, "None")
	features["spines"]				= sanitize_inlist(features["spines"], spines_list, "None")
