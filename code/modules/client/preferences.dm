...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		...
		if(5) // Custom keybindings
			...
			dat += "<style>span.bindings { display: inline-block; width: 200px; } span.independent { display: inline-block; width: 50px } </style><body>"
			...
			for (var/category in kb_categories)
				...
				for (var/i in kb_categories[category])
					...
					if(!length(user_binds[kb.name]))
						...
						dat += "<span class='independent'>Independent Binding: </span> <a href='?_src_=prefs;preference=keybindings_capture;keybinding=[kb.name];old_key=[current_independent_binding];independent=1'>[current_independent_binding]</a>"
						dat += "<br>"
					else
						var/bound_key = user_binds[kb.name][1]
						dat += "<span class='bindings'l>[kb.full_name]</span> <a href ='?_src_=prefs;preference=keybindings_capture;keybinding=[kb.name];old_key=[bound_key]'>[bound_key]</a>"
...
/datum/preferences/proc/CaptureKeybinding(mob/user, datum/keybinding/kb, old_key, independent = FALSE)
	...
	document.onkeyup = function(e) {
		...
		var url = 'byond://?_src_=prefs;preference=keybindings_set;keybinding=[kb.name];old_key=[old_key];independent=[independent];clear_key='+escPressed+';key='+e.key+';alt='+alt+';ctrl='+ctrl+';shift='+shift+';numpad='+numpad+';key_code='+e.keyCode;
		window.location=url;
		deedDone = true;
	}
