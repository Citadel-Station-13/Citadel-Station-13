...
/obj/item/integrated_circuit_printer/Topic(href, href_list)
	...
	if(href_list["build"])
		...
		if(!debug && !materials.use_amount_type(cost, /datum/material/iron))
		...
		switch(href_list["print"])
			...
			if("print")
				...
				if(program["unsupported_circuit"] && !debug)
					...
					var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
					if(debug || materials.use_amount_type(program["metal_cost"], /datum/material/iron))
						cloning = TRUE
						print_program(usr)
					else
					...
					var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
					if(!materials.use_amount_type(program["metal_cost"], /datum/material/iron))
						to_chat(usr, "<span class='warning'>You need [program["metal_cost"]] metal to build that!</span>")
						return
			...
			if("cancel")
				...
				cloning = FALSE
				var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
				materials.use_amount_type(-program["metal_cost"], /datum/material/iron) //use negative amount to regain the cost
