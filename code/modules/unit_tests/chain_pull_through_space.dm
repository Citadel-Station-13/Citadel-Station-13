...
/datum/unit_test/chain_pull_through_space/Run()
	...
	if(alice.loc != run_loc_floor_top_right)
		return Fail("Alice not at needed loc.")
	if(bob.loc != run_loc_floor_top_right)
		return Fail("Bob not at needed loc.")
	if(charlie.loc != run_loc_floor_top_right)
		return Fail("Charlie not at needed loc.")
