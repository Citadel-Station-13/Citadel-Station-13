...
/obj/effect/proc_holder/spell/pointed/InterceptClickOn(mob/living/caller1, params, atom/target)
	if(..())
		return TRUE
	if(aim_assist && isturf(target))
		var/list/possible_targets = list()
		for(var/A in target)
			if(intercept_check(caller1, A, TRUE))
				possible_targets += A
		if(possible_targets.len == 1)
			target = possible_targets[1]
	if(!intercept_check(caller1, target))
		return TRUE
	if(!cast_check(FALSE, caller1))
		return TRUE
	perform(list(target), user = caller1)
	remove_ranged_ability()
