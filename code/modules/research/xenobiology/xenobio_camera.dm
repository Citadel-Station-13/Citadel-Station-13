...
/obj/machinery/computer/camera_advanced/xenobio/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/disk/xenobio_console_upgrade))
		...
		return
	if(istype(O, /obj/item/food/cube/monkey) && (upgradetier & XENOBIO_UPGRADE_MONKEYS)) //CIT CHANGE - makes monkey-related actions require XENOBIO_UPGRADE_MONKEYS
		monkeys++
		...
		for(var/obj/G in P.contents)
			if(istype(G, /obj/item/food/cube/monkey))
				loaded = TRUE
				monkeys++
				qdel(G)
		if(loaded)
