...
/obj/item/slimecross/regenerative/darkpurple/core_effect(mob/living/target, mob/user)
	var/equipped = 0
	equipped += target.equip_to_slot_or_del(new /obj/item/clothing/shoes/sneakers/purple(null), INVENTORY_SLOT_SHOES)
	equipped += target.equip_to_slot_or_del(new /obj/item/clothing/under/color/lightpurple(null), INVENTORY_SLOT_UNIFORM)
	equipped += target.equip_to_slot_or_del(new /obj/item/clothing/gloves/color/purple(null), INVENTORY_SLOT_GLOVES)
	equipped += target.equip_to_slot_or_del(new /obj/item/clothing/head/soft/purple(null), INVENTORY_SLOT_HEAD)
	if(equipped > 0)
...
/obj/item/slimecross/regenerative/darkblue/core_effect(mob/living/target, mob/user)
	...
	if(H.get_item_in_slot(INVENTORY_SLOT_OUTERWEAR))
		fireproofed = TRUE
		var/obj/item/clothing/C = H.get_item_in_slot(INVENTORY_SLOT_OUTERWEAR)
		fireproof(C)
	if(H.get_item_in_slot(INVENTORY_SLOT_HEAD))
		fireproofed = TRUE
		var/obj/item/clothing/C = H.get_item_in_slot(INVENTORY_SLOT_HEAD)
		fireproof(C)
	if(fireproofed)
