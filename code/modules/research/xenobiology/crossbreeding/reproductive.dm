...
/obj/item/slimecross/reproductive/attackby(obj/item/O, mob/user)
	...
	if(istype(O, /obj/item/storage/bag/bio))
		...
		SEND_SIGNAL(O, COMSIG_TRY_STORAGE_TAKE_TYPE, /obj/item/reagent_containers/food/cube/monkey, src, 1, null, null, user, inserted)
		if(inserted.len)
			var/obj/item/reagent_containers/food/cube/monkey/M = inserted[1]
			if(istype(M))
				eat_cube(M, user)
		else
			to_chat(user, "<span class='warning'>There are no monkey cubes in the bio bag!</span>")
	if(istype(O,/obj/item/reagent_containers/food/cube/monkey))
		eat_cube(O, user)
	if(cubes_eaten >= 3)
		var/cores = rand(1,4)
