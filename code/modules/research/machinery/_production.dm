...
/obj/machinery/rnd/production/proc/do_print(path, amount, list/matlist, notify_admins)
	...
	for(var/i in 1 to amount)
		...
		if(efficient_with(I.type))
			...
			I.rnd_crafted(src) // find a better way to do this x2
...
/obj/machinery/rnd/production/proc/user_try_print_id(id, amount)
	if(!istype(linked_console) && requires_console)
		to_chat(usr, "<span class='warning'>You need to link this to a console first!</span>")
		return FALSE
	if(!id)
		return FALSE
	if(istext(amount))
	...
	if(isnull(amount))
		amount = 1
	var/datum/design/D = stored_research.researched_designs[id] ? SSresearch.techweb_design_by_id(id) : null // now we get the GLOBAL tech instead of the console based one, as we never need to change from that AND creating multiple datum holder of tech is bad
	if(!istype(D))
		return FALSE
