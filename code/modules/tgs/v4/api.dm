...
/datum/tgs_api/v4/OnWorldNew(minimum_required_security_level)
	...
	if(!json_file)
		...
		return
	var/cached_json = safe_json_decode(json_file)
	if(!cached_json)
...
/datum/tgs_api/v4/OnTopic(T)
	...
	switch(command)
		...
		if(TGS4_TOPIC_EVENT)
			...
			var/list/event_notification = safe_json_decode(params[TGS4_PARAMETER_DATA])
		...
		if(TGS4_TOPIC_INTEROP_RESPONSE)
			last_interop_response = safe_json_decode(params[TGS4_PARAMETER_DATA])
			return
...
/datum/tgs_api/v4/ChatChannelInfo()
	...
	var/list/json = safe_json_decode(file2text(chat_channels_json_path))
	for(var/I in json)
		. += DecodeChannel(I)
