...
/obj/machinery/airalarm/ui_act(action, params)
	...
	if(. || buildstage != 2)
		return
	if((locked && !hasSiliconAccessInArea(usr)) || (hasSiliconAccessInArea(usr) && aidisabled))
		return
	var/device_id = params["id_tag"]
	switch(action)
		if("lock")
			if(hasSiliconAccessInArea(usr) && !wires.is_cut(WIRE_IDSCAN))
				locked = !locked
				. = TRUE
...
/obj/machinery/airalarm/proc/post_alert(alert_level)
	...
	var/datum/signal/alert_signal = new(list(
		"zone" = get_area_name(src, TRUE, get_base_area = TRUE),
		"type" = "Atmospheric"
	))
	if(alert_level==2)
