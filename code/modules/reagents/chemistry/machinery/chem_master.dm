...
/obj/machinery/chem_master/ui_act(action, params)
	...
	if(action == "create")
		...
		if(item_type == "condimentPack")
			var/obj/item/food/condiment/pack/P
			for(var/i = 0; i < amount; i++)
				P = new/obj/item/food/condiment/pack(drop_location())
				P.originalname = name
		...
		if(item_type == "condimentBottle")
			var/obj/item/food/condiment/P
			for(var/i = 0; i < amount; i++)
				P = new/obj/item/food/condiment(drop_location())
				P.originalname = name
				P.name = trim("[name] bottle")
				reagents.trans_to(P, vol_each)//, transfered_by = usr)
			return TRUE
