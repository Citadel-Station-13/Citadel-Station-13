...
/datum/reagent/medicine/liquid_solder
	name = "Liquid Solder"
	description = "Liquid solder that fixes damaged positronic brains. Mildly toxic to organics."
	reagent_state = LIQUID
	color = "#302D28"
	metabolization_rate = 0.2 * REAGENTS_METABOLISM

/datum/reagent/medicine/liquid_solder/on_mob_life(mob/living/carbon/M)
	. = ..()
	if(HAS_TRAIT(M, TRAIT_ROBOTIC_ORGANISM))
...
/datum/reagent/medicine/nanite_slurry
	name = "Nanite Slurry"
	description = "Small Nanites that fix dents in synthetics from the inside."
	reagent_state = LIQUID
	color = "#B5A998"
	metabolization_rate = 0.2 * REAGENTS_METABOLISM
	overdose_threshold = 30
...
/datum/reagent/medicine/nanite_slurry/overdose_process(mob/living/carbon/M)
	. = ..()
	if(HAS_TRAIT(M, TRAIT_ROBOTIC_ORGANISM))
		M.adjustBruteLoss(4*REM) //inverts its positive effect on overdose, for organics it's just more toxic
	else
		M.adjustToxLoss(0.5)

/datum/reagent/medicine/smartwire_slurry
	name = "Smartwire Slurry"
	description = "Smart, liquid wires that fix burnt wiring in synthetics."
	reagent_state = LIQUID
	color = "#DEA04E"
	metabolization_rate = 0.2 * REAGENTS_METABOLISM
	overdose_threshold = 30

/datum/reagent/medicine/smartwire_slurry/on_mob_life(mob/living/carbon/M)
	. = ..()
	if(HAS_TRAIT(M, TRAIT_ROBOTIC_ORGANISM))
...
/datum/reagent/medicine/smartwire_slurry/overdose_process(mob/living/carbon/M)
	. = ..()
	if(HAS_TRAIT(M, TRAIT_ROBOTIC_ORGANISM))
		M.adjustFireLoss(4*REM) //inverts its positive effect on overdose, for organics it's just more toxic
	else
		M.adjustToxLoss(0.5)
