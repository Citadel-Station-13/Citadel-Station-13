...
/datum/controller/subsystem/job/proc/RecomputePreferencesRender()
	...
	for(var/datum/job/job as anything in GetAllJobs())
		...
		var/datum/department/D = (job.preferred_preferences_department && GetDepartmentType(job.preferred_preferences_department)) || job.GetPrimarySupervisedDepartment() || job.GetPrimaryDepartment()
		var/list/dept
		if(faction[D])
			dept = faction[D]
		else
	...
	for(var/faction as anything in horrifying_nested_list)
		...
		for(var/datum/department/D as anything in L1)
			...
			if(headname in L2)
				...
				L2[headname] = _head
			// sort the rest
			sortTim(L2, /proc/cmp_text_asc, associative = FALSE, fromIndex = 2)
	// finish
