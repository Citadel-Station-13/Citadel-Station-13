	...
	for(var/mob/dead/new_player/player in GLOB.player_list)
		if(player.ready == PLAYER_READY_TO_PLAY && player.mind)
			...
			if(player.new_character && player.client && player.client.prefs) // we cannot afford a runtime, ever
				LAZYOR(player.client.persistent_variables.slots_joined_as, player.client.prefs.default_slot)
				LAZYOR(player.client.persistent_variables.characters_joined_as, player.new_character.real_name)
			else
