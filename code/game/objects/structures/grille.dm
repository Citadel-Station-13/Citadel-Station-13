	...
	if(isobj(AM))
		if(prob(50) && anchored && !broken)
			...
			if(C)
				...
				if(ismob(AM)) // Citadel edit, removes exploit with shit code by tg
					tesla_zap(src, 3, C.powernet.avail * 0.05) //Zap for 5/100 of the amount of power. At a million watts in the grid, it will be as powerful as a tesla revolver shot.
