/obj/item/clothing/head/bowlersharp
	name = "bowler-hat"
...
/obj/item/clothing/head/bowlersharp/Initialize()
	. = ..()

/obj/item/clothing/head/bowlersharp/throw_at(atom/target, range, speed, mob/thrower, spin=FALSE, diagonals_first = FALSE, datum/callback/callback)
	unlimitedthrow = 1
	. = ..(target, range, speed, thrower, FALSE, diagonals_first, callback)

/obj/item/clothing/head/bowlersharp/throw_impact(atom/hit_atom)
	if(!ishuman(hit_atom))//if the ball hits a nonhuman
		unlimitedthrow = 0
		return ..()
	var/mob/living/carbon/human/H = hit_atom
	if(iscarbon(H))
		H.Knockdown(8) //Heres the downfall of this, they need to go down.
