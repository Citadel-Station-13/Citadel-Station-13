...
/obj/item/proc/help_light_cig(mob/living/M)
	var/mask_item = M.get_item_in_slot(INVENTORY_SLOT_MASK)
...
/obj/item/clothing/mask/cigarette/Initialize()
	...
	if(starts_lit)
		light()
	AddComponent(/datum/component/knockoff,90,list(BODY_ZONE_PRECISE_MOUTH),list(INVENTORY_SLOT_MASK))//90% to knock off when wearing a mask
...
/obj/item/clothing/mask/vape/dropped(mob/user)
	...
	if(C.get_item_in_slot(INVENTORY_SLOT_MASK) == src)
