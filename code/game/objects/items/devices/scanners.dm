...
/proc/healthscan(mob/user, mob/living/M, mode = 1, advanced = FALSE)
	...
	if(iscarbon(M))
		...
		for(var/organ in C.internal_organs)
			...
			//GENERAL HANDLER
			if(!damage_message)
				if(O.organ_flags & ORGAN_FAILING) //Internally it's referred to as endstage everywhere - this is to make the code consistent with the in game messages. If people get mad I will change it later
