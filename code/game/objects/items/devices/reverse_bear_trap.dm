...
/obj/item/reverse_bear_trap/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
	if(iscarbon(user))
		...
		if(C.get_item_in_slot(INVENTORY_SLOT_HEAD) == src)
...
/obj/item/reverse_bear_trap/attack(mob/living/target, mob/living/user)
	if(target.get_item_in_slot(INVENTORY_SLOT_HEAD))
	...
	if(!do_after(user, 30, target = target) || target.get_item_in_slot(INVENTORY_SLOT_HEAD))
		return
	...
	target.equip_to_slot_if_possible(src, INVENTORY_SLOT_HEAD)
	arm()
...
/obj/item/reverse_bear_trap/proc/snap()
	...
	if(!istype(H) || H.get_item_in_slot(INVENTORY_SLOT_HEAD) != src)
