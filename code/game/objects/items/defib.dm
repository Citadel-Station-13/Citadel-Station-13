...
/obj/item/defibrillator/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
	if(loc == user)
		if(slot_flags == ITEM_SLOT_BACK)
			if(user.get_item_in_slot(INVENTORY_SLOT_BACK) == src)
				ui_action_click()
			else
				to_chat(user, "<span class='warning'>Put the defibrillator on your back first!</span>")

		else if(slot_flags == ITEM_SLOT_BELT)
			if(user.get_item_in_slot(INVENTORY_SLOT_BELT) == src)
				ui_action_click()
			else
...
/obj/item/defibrillator/MouseDrop(obj/over_object)
	...
	if(!. && ismob(loc) && loc == usr)
		var/mob/M = loc
		if(!M.incapacitated() && istype(over_object, /obj/screen/hand))
...
/obj/item/defibrillator/equipped(mob/user, slot)
	..()
	if((slot_flags == ITEM_SLOT_BACK && slot != INVENTORY_SLOT_BACK) || (slot_flags == ITEM_SLOT_BELT && slot != INVENTORY_SLOT_BELT))
		remove_paddles(user)
		update_power()
