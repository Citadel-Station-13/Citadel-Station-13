...
/obj/machinery/recharger/on_attack_hand(mob/user)
	...
	if(charging)
		...
		if(istype(charging, /obj/item/ammo_casing/mws_batt))
			...
			if(R.cell.charge < R.cell.maxcharge)
				R.cell.give(R.cell.chargerate * recharge_coeff * delta_time / 2)
				use_power(125 * recharge_coeff * delta_time)
				using_power = 1
			if(R.BB == null)
				R.chargeshot()
		...
		if(istype(charging, /obj/item/ammo_box/magazine/mws_mag))
			...
			for(var/B in R.stored_ammo)
				...
				if(batt.cell.charge < batt.cell.maxcharge)
					batt.cell.give(batt.cell.chargerate * recharge_coeff * delta_time / 2)
					use_power(125 * recharge_coeff * delta_time)
					using_power = 1
				if(batt.BB == null)
					batt.chargeshot()
...
/obj/machinery/recharger/update_overlays()
	. = ..()
	if(machine_stat & (NOPOWER|BROKEN) || !anchored)
		. += mutable_appearance(icon, "rechargeroff", layer, plane, alpha)
		return
	if(panel_open)
		. += mutable_appearance(icon, "rechargeropen", layer, plane, alpha)
		return

	if(!charging)
		. += mutable_appearance(icon, "recharger0", layer, plane, alpha)
		. += mutable_appearance(icon, "recharger0", 0, EMISSIVE_PLANE, alpha)
		return
	if(using_power)
		. += mutable_appearance(icon, "recharger1", layer, plane, alpha)
		. += mutable_appearance(icon, "recharger1", 0, EMISSIVE_PLANE, alpha)
		return

	. += mutable_appearance(icon, "recharger2", layer, plane, alpha)
	. += mutable_appearance(icon, "recharger2", 0, EMISSIVE_PLANE, alpha)
