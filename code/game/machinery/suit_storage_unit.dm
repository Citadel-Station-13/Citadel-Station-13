	...
	if(uv_cycles)
		...
		if(uv_super)
			...
			for(var/am in things_to_clear) //Scorches away blood and forensic evidence, although the SSU itself is unaffected
				...
				SEND_SIGNAL(dirty_movable, COMSIG_COMPONENT_CLEAN_ACT, CLEAN_STRONG)
				dirty_movable.clean_blood()
				dirty_movable.fingerprints = null
				var/datum/component/radioactive/contamination = dirty_movable.GetComponent(/datum/component/radioactive)
				if(contamination)
					qdel(contamination)
		open_machine(FALSE)
		if(mob_occupant)
