...
/obj/machinery/computer/atmos_control/tank/ui_data(mob/user)
	...
	data["maxInputRate"] = input_info ? ATMOSMECH_PUMP_RATE : 0
	data["outputting"] = output_info ? output_info["power"] : FALSE
	data["outputPressure"] = output_info ? output_info["internal"] : 0
	data["maxOutputPressure"] = output_info ? ATMOSMECH_PUMP_PRESSURE_SANE : 0
	return data
...
/obj/machinery/computer/atmos_control/tank/ui_act(action, params)
	...
	switch(action)
		...
		if("rate")
			...
			if(!isnull(target))
				target = clamp(target, 0, ATMOSMECH_PUMP_RATE)
		...
		if("pressure")
			...
			if(!isnull(target))
				target = clamp(target, 0, ATMOSMECH_PUMP_PRESSURE_SANE)
