...
/obj/machinery/dna_scannernew/update_icon_state()
	...
	if(machine_stat & (NOPOWER|BROKEN))
		icon_state = "[initial(icon_state)][state_open ? "_open" : null]_unpowered"
		return ..()

	if((machine_stat & MAINT) || panel_open)
		icon_state = "[initial(icon_state)][state_open ? "_open" : null]_maintenance"
		return ..()
...
/obj/machinery/dna_scannernew/MouseDrop_T(mob/target, mob/user)
	if(user.stat != CONSCIOUS || !Adjacent(user) || !user.Adjacent(target) || !iscarbon(target) || !user.IsAdvancedToolUser()) // || HAS_TRAIT(user, TRAIT_UI_BLOCKED)
		return
