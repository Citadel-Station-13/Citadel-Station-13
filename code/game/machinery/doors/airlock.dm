	...
	switch(state)
		if(AIRLOCK_CLOSED)
			...
			if(obj_integrity < integrity_failure * max_integrity)
				damag_overlay = get_airlock_overlay("sparks_broken", overlays_file, null, ABOVE_LIGHTING_PLANE)
			else if(obj_integrity < (0.75 * max_integrity))
				damag_overlay = get_airlock_overlay("sparks_damaged", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(lights && hasPower())

				if(locked)
					lights_overlay = get_airlock_overlay("lights_bolts", overlays_file, null, ABOVE_LIGHTING_PLANE)
				else if(emergency)
					lights_overlay = get_airlock_overlay("lights_emergency", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(note)
		...
		if(AIRLOCK_DENY)
			...
			if(welded)
				weld_overlay = get_airlock_overlay("welded", overlays_file)
			lights_overlay = get_airlock_overlay("lights_denied", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(note)
				note_overlay = get_airlock_overlay(notetype, note_overlay_file)

		if(AIRLOCK_EMAG)
			frame_overlay = get_airlock_overlay("closed", icon)
			sparks_overlay = get_airlock_overlay("sparks", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(airlock_material)
				filling_overlay = get_airlock_overlay("[airlock_material]_closed", overlays_file)
			else
				filling_overlay = get_airlock_overlay("fill_closed", icon)
		...
		if(AIRLOCK_CLOSING)
			...
			if(lights && hasPower())
				lights_overlay = get_airlock_overlay("lights_closing", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(panel_open)
				if(security_level)
		...
		if(AIRLOCK_OPEN)
			...
			if(obj_integrity < (0.75 * max_integrity))
				damag_overlay = get_airlock_overlay("sparks_open", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(note)
		...
		if(AIRLOCK_OPENING)
			...
			if(lights && hasPower())
				lights_overlay = get_airlock_overlay("lights_opening", overlays_file, null, ABOVE_LIGHTING_PLANE)
			if(panel_open)
				if(security_level)
