...
/obj/machinery/door/airlock/proc/change_paintjob(obj/item/airlock_painter/painter, mob/user)
	...
	if((!in_range(src, usr) && src.loc != usr) || !painter.use_paint(user))
		return
	switch(paintjob)
		if("Standard")
			icon = 'icons/obj/doors/airlocks/station/public.dmi'
...
/obj/machinery/door/airlock/obj_break(damage_flag)
	// due to "code bad" i have to do this
...
/obj/machinery/door/airlock/proc/set_wires()
	return  new wiretypepath(src) //CIT CHANGE - makes it possible for airlocks to have different wire datums
