...
/obj/machinery/cryopod
	...
	var/time_till_despawn = 15 MINUTES // This is reduced to 30 seconds if a player manually enters cryo
...
/obj/machinery/cryopod/proc/despawn_occupant()
	...
	if(mob_occupant.mind && mob_occupant.mind.assigned_role)
		...
		crew_member["job"] = job
		SSjob.FreeRole(job)
		if(LAZYLEN(mob_occupant.mind.special_role))
	...
	if(!mob_occupant.get_ghost(TRUE))
		if(world.time < 15 MINUTES) // before the 15 minute mark
			mob_occupant.ghostize(FALSE, penalize = TRUE, voluntary = TRUE, cryo = TRUE) // Players despawned too early may not re-enter the game
		else
			mob_occupant.ghostize(FALSE, penalize = FALSE, voluntary = TRUE, cryo = TRUE)
	handle_objectives()
	QDEL_NULL(occupant)
	open_machine()
	name = initial(name)
