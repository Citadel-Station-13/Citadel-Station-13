...
/datum/game_mode/proc/add_thrall(datum/mind/new_thrall_mind)
	...
	if(!(new_thrall_mind in shadowling_thralls))
		...
		if(jobban_isbanned(new_thrall_mind.current, ROLE_SHADOWLING) || jobban_isbanned(new_thrall_mind.current, ROLE_TRAITOR
...
/datum/species/shadow/shadowling
	//Normal shadowpeople but with enhanced effects. Prepare to get aids with bad code
	name = "Shadowling"
	id = "shadowling"
	limbs_id = "shadow"
	species_traits = list(NOBLOOD, NOBREATH, RADIMMUNE, NOGUNS) //Can't use guns due to muzzle flash
	burnmod = 1.5 //1.5x burn damage, 2x is excessive
	E.flash_protect = -1 //Makes them more vulnerable to flashes and flashbangs

/datum/species/shadow/shadowling/handle_life(var/mob/living/carbon/human/H)
	var/light_amount = 0
	...
	if(isturf(H.loc))
		...
		if(light_amount > LIGHT_DAM_THRESHOLD && !H.incorporeal_move) //Can survive in very small light levels. Also doesn't take damage while incorporeal, for shadow walk purposes
			...
			H.adjustCloneLoss(-1)
			// there should be a stun/weaken remove, dunno how to

/datum/species/shadow/shadowling/lesser //Empowered thralls. Obvious, but powerful
	name = "Lesser Shadowling"
	id = "lesser"

	species_traits = list(NOBLOOD, NOBREATH, RADIMMUNE)
	burnmod = 1.1
