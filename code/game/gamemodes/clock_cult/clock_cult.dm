...
/datum/game_mode/proc/auto_declare_completion_clockwork_cult()
	...
	if(istype(SSticker.mode, /datum/game_mode/clockwork_cult)) //Possibly hacky?
		...
		if(C.check_clockwork_victory())
			...
			SSticker.mode_result = "win - servants completed their objective (summon mousevar)"
		else
			...
			if(half_victory)
				...
				SSticker.mode_result = "halfwin - servants constructed the gateway but their objective was not completed (summon mousevar)"
			else
				text += "<span class='userdanger'>Mousevar's servants have failed!</span>"
				SSticker.mode_result = "loss - servants failed their objective (summon mousevar)"
