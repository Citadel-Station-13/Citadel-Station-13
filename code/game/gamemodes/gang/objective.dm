...
// because this is meant for the gang gamemode, which is a bloodless coup, unlike revs.
...
/datum/objective/gang

/datum/objective/gang/check_completion()
	return get_points() ? TRUE : FALSE

/datum/objective/gang/proc/get_points()
	return 0 // int, not bool

/datum/objective/gang/proc/result_points(datum/mind/the_dude, base_points) // App
	var/initial_points = base_points
	if(the_dude)
		var/datum/antagonist/gang/O = the_dude.has_antag_datum(/datum/antagonist/gang)
...
// Heads gang objective. This targets the heads only, assigning points based on the rank of the head, captain being the highest target.
/datum/objective/gang/heads
...
/datum/objective/gang/heads/proc/find_targets()
...
/datum/objective/gang/heads/update_explanation_text()
	if(targets.len)
...
/datum/objective/gang/heads/check_completion()
	if(!targets.len)
		return TRUE
	. = ..()

// Amount of points = foreach head, result += head basepoints * modifier.
/datum/objective/gang/heads/get_points()
	var/base_points = 0
...
// AI converting objective. The team who managed to convert the AI with the gang module gets the normal 1.5x boost.
/datum/objective/gang/AI
...
/datum/objective/gang/AI/get_points() // If you simply kill the Ai you get nothing, you need it to gang the heads.
	...
	for(var/i in GLOB.ai_list)
		...
		if(AI.mind)
			...
			var/datum/antagonist/gang/O = M.has_antag_datum(/datum/antagonist/gang)
			if(M)
				. += (O.team == team) ? AIPTS*CONVERTED_OURS : AIPTS

/datum/objective/gang/AI/update_explanation_text()
	if(!GLOB.ai_list.len)
		explanation_text = "Nothing."
	else
		explanation_text = "Enslave the AIs to your team using the special AI module board in your storage implant. It is required you use said module."

/datum/objective/gang/AI/check_completion()
	if(!GLOB.ai_list.len)
		return TRUE
	. = ..()
...
/datum/objective/gang/target

/datum/objective/gang/target/update_explanation_text()
	if(target)
...
/datum/objective/gang/target/is_unique_objective(datum/mind/possible_target)
	if(possible_target.assigned_role in GLOB.command_positions)
		return FALSE
	return TRUE

/datum/objective/gang/target/check_completion()
	if(!target)
		return TRUE
	. = ..()

/datum/objective/gang/target/get_points()
