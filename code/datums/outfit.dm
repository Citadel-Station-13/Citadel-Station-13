...
/datum/outfit/proc/apply_fingerprints(mob/living/carbon/human/H)
	if(!istype(H))
		return
	for(var/obj/item/I as anything in H.GetAllItems(FALSE))
		I.add_fingerprint(H, ignoregloves = TRUE)
		var/list/L = list()
		SEND_SIGNAL(I, COMSIG_TRY_STORAGE_RETURN_INVENTORY, L)
		if(L.len)
			for(var/obj/item/nested in L)
				nested.add_fingerprint(H, ignoregloves = TRUE)
	return TRUE
