...
/datum/disease/cold/stage_act()
	...
	switch(stage)
		if(2)
			if(affected_mob.resting && prob(40))  //changed FROM prob(10) until sleeping is fixed
		...
		if(3)
			if(affected_mob.resting && prob(25))  //changed FROM prob(5) until sleeping is fixed
				to_chat(affected_mob, "<span class='notice'>You feel better.</span>")
				cure()
				return
			if(prob(1) && prob(1))
