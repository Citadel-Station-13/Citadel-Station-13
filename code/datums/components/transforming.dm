...
/datum/component/transforming/proc/default_transform_message(obj/item/source, mob/user)
	to_chat(user, "[active ? "enabled" : "disabled"] [source]")
...
/datum/component/transforming/proc/clumsy_transform_effect(mob/living/user)
	...
	if(active && prob(50))
		var/hurt_self_verb_simple = LAZYLEN(attack_verb_on) ? pick(attack_verb_on) : "hit"
		var/hurt_self_verb_continuous = LAZYLEN(attack_verb_on) ? pick(attack_verb_on) : "hits"
