/datum/component/range_protection
...
/datum/component/range_protection/Initialize(dist, r_or_v, comsig, value, _callback, whitelist, blacklist)
	. = ..()
...
/datum/component/range_protection/proc/on_atom_entered(turf/T, atom/movable/entered, atom/oldloc)
...
/datum/component/range_protection/proc/on_atom_exited(turf/source, atom/movable/exited, atom/newloc)
	if(exited == parent) //Reorganize the list of "protected" atoms
		...
		var/list/turf_sigs = list(COMSIG_ATOM_ENTERED, COMSIG_ATOM_EXITED, COMSIG_TURF_CHANGE)
...
/datum/component/range_protection/proc/transfer_sigs(turf/T, path, list/new_baseturfs, flags, list/transferring_comps, list/callbacks)
	callbacks += CALLBACK(src, .proc/register_changed_turf, CHANGETURF_PROTOTYPE)
	range_view_turf_list -= T

/datum/component/range_protection/proc/register_changed_turf(turf/T)
	RegisterSignal(T, COMSIG_ATOM_ENTERED, .proc/on_atom_entered)
	RegisterSignal(T, COMSIG_ATOM_EXITED, .proc/on_atom_exited)
	RegisterSignal(T, COMSIG_TURF_CHANGE, .proc/transfer_sigs)
	range_view_turf_list += T

/datum/component/range_protection/proc/return_value(atom/source)
...
/obj/structure/range_test/ComponentInitialize()
	. = ..()
	AddComponent(/datum/component/range_protection, comsig = COMSIG_ATOM_EMP_ACT, value = EMP_PROTECT_CONTENTS|EMP_PROTECT_WIRES|EMP_PROTECT_SELF)
