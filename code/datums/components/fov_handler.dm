...
/datum/component/fov_handler/Initialize(fov_type = FOV_180_DEGREES)
	...
	blocker_mask = new
	visual_shadow = new
	visual_shadow.alpha = parent_client?.prefs.fov_darkness
...
/datum/component/fov_handler/RegisterWithParent()
	...
	RegisterSignal(parent, COMSIG_MOB_DEATH, .proc/update_mask)
	...
	RegisterSignal(parent, COMSIG_MOB_CLIENT_LOGOUT, .proc/mob_logout)

/datum/component/fov_handler/UnregisterFromParent()
	. = ..()
	UnregisterSignal(parent, list(COMSIG_MOB_RESET_PERSPECTIVE, COMSIG_ATOM_DIR_CHANGE, COMSIG_MOB_DEATH, COMSIG_LIVING_REVIVE, COMSIG_MOB_CLIENT_LOGOUT))
