...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_FACIAL_HAIR_STYLE_BLOCK] 	= construct_block(facial_hair_styles_list.Find(H.facial_hair_style), facial_hair_styles_list.len)
		L[DNA_COCK_BLOCK] 				= sanitize_integer(features["has_cock"])//num,min,max,def		defaults are (num, 0, 1, 0)
		...
		L[DNA_COCK_SIZE_BLOCK] 			= sanitize_integer(features["cock_size"], COCK_SIZE_MIN, COCK_SIZE_MAX, COCK_SIZE_NORMAL)//num,min,max,def
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_COCK_BLOCK)
			setblock(uni_identity, blocknumber, sanitize_integer(features["has_cock"])
		if(DNA_COCK_SHAPE_BLOCK)
			setblock(uni_identity, blocknumber, construct_block(cock_types_list.Find(features["cock_shape"]), cock_types_list.len))
		if(DNA_COCK_COLOR_BLOCK)
			setblock(uni_identity, blocknumber, construct_block(Find(features["cock_color"]))
