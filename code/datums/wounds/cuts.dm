...
/datum/wound/brute/cut/proc/lick_wounds(mob/living/carbon/human/user)
	...
	blood_flow -= 0.5
	if( ismoth(victim) || isfelinid(victim) || ismonkey(victim) ) // Yep you can lick monkeys.
		user.reagents.add_reagent(/datum/reagent/hairball, 2)

	else if( ishumanbasic(victim) || isflyperson(victim) || islizard(victim) || isdullahan(victim) || ismammal(victim) || isavian(victim) )
