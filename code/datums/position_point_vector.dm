...
//Don't use this for anything that you don't absolutely have to use this with (like projectiles!) because it isn't worth using a datum unless you need accuracy down to decimal places in steps.
...
/datum/position			//For positions with map x/y/z and step x/y so you don't have to return lists. Could use addition/subtraction in the future I guess.
	var/x = 0
	var/y = 0
	var/z = 0
	var/step_x = 0
...
/datum/position/proc/valid()
	return x && y && z && !isnull(step_x) && !isnull(step_y)

/datum/position/New(_x = 0, _y = 0, _z = 0, _step_x = 0, _step_y = 0)	//first argument can also be a /datum/point.
...
/datum/position/proc/return_sx()
	return step_x

/datum/position/proc/return_sy()
	return step_y

/datum/position/proc/return_point()
	return new /datum/point(src)

/datum/point		//A precise point on the map in absolute step locations based on world.icon_size. steps are FROM THE EDGE OF THE MAP!
	var/x = 0
	var/y = 0
	var/z = 0
...
/datum/point/New(_x, _y, _z, _step_x = 0, _step_y = 0)	//first argument can also be a /datum/position or /atom.
...
/datum/point/proc/debug_out()
	...
	return "\ref[src] aX [x] aY [y] aZ [z] pX [return_sx()] pY [return_sy()] mX [T.x] mY [T.y] mZ [T.z]"
...
/datum/point/proc/return_sx()
	return MODULUS(x, world.icon_size) - 16 - 1

/datum/point/proc/return_sy()
...
/datum/point/vector
	...
	var/mpy = 0
	var/starting_x = 0			//just like before, steps from EDGE of map! This is set in initialize_location().
	var/starting_y = 0
	var/starting_z = 0
...
/datum/point/vector/processed		//step_speed is per decisecond.
	var/last_process = 0
	var/last_move = 0
	var/paused = FALSE
