...
#define BEING_SHOCKED			(1<<2)	//Whether this thing is currently (already) being shocked by a tesla
...
#define FROZEN					(1<<6)
#define SHOVABLE_ONTO			(1<<7)	//called on turf.shove_act() to consider whether an object should have a niche effect (defined in their own shove_act()) when someone is pushed onto it, or do a sanity CanPass() check.
#define BLOCK_Z_FALL			(1<<8)
