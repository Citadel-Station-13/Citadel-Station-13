...
/datum/controller/subsystem/ticker/proc/law_report()
	...
	for (var/i in GLOB.ai_list)
		...
		if(aiPlayer.mind)
			parts += "<b>[aiPlayer.name]</b> (Played by: <b>[aiPlayer.mind.hide_ckey ? get_fake_ckey() : aiPlayer.mind.key]</b>)'s laws [aiPlayer.stat != DEAD ? "at the end of the round" : "when it was <span class='redtext'>deactivated</span>"] were:"
		...
		if (aiPlayer.connected_robots.len)
			...
			for(var/mob/living/silicon/robot/robo in aiPlayer.connected_robots)
				borg_num--
				if(robo.mind)
					robolist += "<b>[robo.name]</b> (Played by: <b>[robo.mind.hide_ckey ? get_fake_ckey() : robo.mind.key]</b>)[robo.stat == DEAD ? " <span class='redtext'>(Deactivated)</span>" : ""][borg_num ?", ":""]<br>"
			parts += "[robolist]"
		if(!borg_spacer)
			borg_spacer = TRUE
	...
	for (var/mob/living/silicon/robot/robo in GLOB.silicon_mobs)
		if (!robo.connected_ai && robo.mind)
			parts += "[borg_spacer?"<br>":""]<b>[robo.name]</b> (Played by: <b>[robo.mind.hide_ckey ? get_fake_ckey() : robo.mind.key]</b>) [(robo.stat != DEAD)? "<span class='greentext'>survived</span> as an AI-less borg!" : "was <span class='redtext'>unable to survive</span> the rigors of being a cyborg without an AI."] Its laws were:"
...
/proc/printplayer(datum/mind/ply, fleecheck)
	...
	var/text = "<b>[ply.hide_ckey ? get_fake_ckey() : ply.key]</b> was <b>[ply.name]</b>[jobtext] and"
	if(ply.current)
...
//proc for making a fake ckey for people with hidden ckeys, just a first and last name with a number from 1-99 in it
/proc/get_fake_ckey()
